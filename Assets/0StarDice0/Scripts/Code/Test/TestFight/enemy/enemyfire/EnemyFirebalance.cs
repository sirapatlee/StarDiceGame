...
public class EnemyFirebalance : MonoBehaviour
{
    [Header("Player Setup")]
...
public PlayerState player;
         ...
         if (lightBuffTurnsLeft > 0)//สกิล 2 แสง
    {
        ...
        enemyHP -= finalDamage;
        //enemyHP -= finalWaterDamage;
        enemyHP = Mathf.Clamp(enemyHP, 0, 150);
        StartCoroutine(ShakeEnemy()); // ศัตรูสั่น
    ...
    public ItemID FireDagger = ItemID.FireDagger; // สมมติกล่องนี้ดรอปดาบไฟ
    public ItemID FireAxe = ItemID.FireAxe; 
    public ItemID FireArmor = ItemID.FireArmor; 
    ...
    public void OpenChest()
    {
        // เรียกใช้คำสั่งปลดล็อก
         int roll = Random.Range(1, 101);
            ...
            if (roll < 21)
            {
                 EquipmentManager.Instance.UnlockItem(FireDagger);
                  showImage.sprite = itemImages[0]; 
                 showImage.gameObject.SetActive(true);
            }
            else if (roll < 41 && roll > 20)
        {
             EquipmentManager.Instance.UnlockItem(FireAxe);
              showImage.sprite = itemImages[1]; 
                 showImage.gameObject.SetActive(true);
        }
          else if (roll < 61 && roll > 40)
        {
             EquipmentManager.Instance.UnlockItem(FireArmor);
 ...
 return; // ⭐️ สำคัญมาก: ออกจากฟังก์ชันนี้ทันที (ไม่ไปเปิดปุ่มด้านล่าง)
  ...
  GameEventManager.TryAddCount1(1);
    ...
    void ChooseAndExecuteBestEnemyMove(int basicAttackDamage)
    {
        ...
        {
            Debug.Log("AI เลือก: Skill 3 (fire)");
            EnemySkill3();
        }
