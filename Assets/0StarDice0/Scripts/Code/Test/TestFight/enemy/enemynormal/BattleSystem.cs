...
StartCoroutine(DelayedEnemyTurn()); 
 ...
 GameEventManager.TryAddCount1(1);
    ...
    IEnumerator FinalizeTurnAfterDelay()
    {
        ...
        Debug.Log("AI: รอครบ 3 วิแล้ว, เริ่มเทิร์นผู้เล่น");
    ...
    float CalculateSkill1Score() // สกิล 1: โจมตี (Damage)
    {
        ...
        // ถ้าผู้เล่นชนะทาง (ธาตุ Water) ให้คะแนนน้อยๆ
    ...
    float CalculateSkill2Score() // สกิล 2: heal
    {
        if (enemySkill2Cooldown > 0) return -1000; // ใช้ไม่ได้

        float score = 30; // ค่าพื้นฐานของ b
        
        if (enemyHP > (enemyMaxHP * 0.9f)) // ถ้าเลือดมากกว่า 90%
            score -= 30; 
    ...
    float CalculateSkill3Score() // สกิล 3: สตั๊น 
    {
    ...
    void EnemyUseSkill1() //<--สกิล 1 ศัตรู
    {
        ...
        {
            damage /= 2;
            Debug.Log("ผู้เล่นเป็นธาตุน้ำ ดาเมจถูกลดครึ่งหนึ่ง");
        }
        else if (selectedPlayer.elementType == ElementType.Wind)
        {
            damage *= 2;
            Debug.Log("ผู้เล่นเป็นธาตุลม ดาเมจคูณ 2");
        }
        ...
        Debug.Log("ศัตรูใช้ Fire Claw ทำดาเมจ " + damage);
    }
    ...
    void EnemySkill3() //<--สกิล 3 ศัตรู
    {
        ...
        Debug.Log("ศัตรูใช้สกิล 3 หยุดผู้เล่น 3 เทิร์น และโจมตีทันที 3 ครั้งด้วยสกิล");
