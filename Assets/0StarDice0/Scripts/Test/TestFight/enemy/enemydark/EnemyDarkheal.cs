...
public class EnemyDarkheal: MonoBehaviour
{
    [Header("Player Setup")]
    ...
    void PlaySoundEffect(int index)
{
  ...
  void UseSkill(SkillData skill)
    ...
    public ItemID DarkDagger = ItemID.DarkDagger; // สมมติกล่องนี้ดรอปดาบไฟ
    public ItemID DarkShoes = ItemID.DarkShoes; 
    public ItemID DarkRing = ItemID.DarkRing; 
    ...
    public void OpenChest()
    {
        // เรียกใช้คำสั่งปลดล็อก
         int roll = Random.Range(1, 101);
            ...
            if (roll < 21)
            {
                 EquipmentManager.Instance.UnlockItem(DarkDagger);
                 showImage.sprite = itemImages[0]; 
                 showImage.gameObject.SetActive(true);
            }
            else if (roll < 41 && roll > 20)
        {
             EquipmentManager.Instance.UnlockItem(DarkShoes);
             showImage.sprite = itemImages[1]; 
                 showImage.gameObject.SetActive(true);
        }
          else if (roll < 61 && roll > 40)
        {
             EquipmentManager.Instance.UnlockItem(DarkRing);
 ...
 return; // ⭐️ สำคัญมาก: ออกจากฟังก์ชันนี้ทันที (ไม่ไปเปิดปุ่มด้านล่าง)
    ...
    float CalculateSkill1Score() // สกิล 1: โจมตี (Damage)
    {
       if (enemySkill1Cooldown > 0) return -1000; // ใช้ไม่ได้
        ...
        if (selectedPlayer.elementType == ElementType.Light)
            score *= 2; 
    ...
    {
        enemyHPBar.maxValue = 200;
        enemyHPBar.value = enemyHP;
        enemyHPText.text = $"HP: {enemyHP}/200";
    }
 ...
 void EnemyUseSkill1() //<--สกิล 1 ศัตรู
    {
        ...
        {
            damage *= 2;
            Debug.Log("ผู้เล่นเป็นธาตุแสง ดาเมจคูณ 2");
        }
    ...
    void EnemySkill3() //<--สกิล 3 ศัตรู
    {
        ...
        EnemyDarkWalk80 =  Random.Range(1,4);
