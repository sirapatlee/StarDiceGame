...
public class EnemyEarthheal: MonoBehaviour
{
    [Header("Player Setup")]
...
public PlayerState player;
    ...
    public Image showImage;
     public ItemID EarthArmor = ItemID.EarthArmor; // สมมติกล่องนี้ดรอปดาบไฟ
    public ItemID EarthHammer = ItemID.EarthHammer; 
    public ItemID EarthRing = ItemID.EarthRing; 
...
StartCoroutine(DelayedEnemyTurn()); 
 ...
 return; // ⭐️ สำคัญมาก: ออกจากฟังก์ชันนี้ทันที (ไม่ไปเปิดปุ่มด้านล่าง)
  ...
  GameEventManager.TryAddCount1(1);
    ...
    {
        enemyHPBar.maxValue = 180;
        enemyHPBar.value = enemyHP;
        enemyHPText.text = $"HP: {enemyHP}/180";
    }
 ...
 void EnemyUseSkill1() //<--สกิล 1 ศัตรู
    {
        ...
        Debug.Log("ศัตรูใช้ earth ทำดาเมจ " + damage);
    }
    ...
    void EnemySkill3() //<--สกิล 3 ศัตรู
    {
        ...
        Debug.Log($"ศัตรูใช้สกิล shield 100%");
    ...
    IEnumerator EnemyTripleSkillTurn()
    {
        for (int i = 1; i <= 1; i++)
        {
            ...
            if (enemySkill2Cooldown == 0)
            {
                EnemySkill2();
                usedSkill = true;
            }
            
            else if (enemySkill1Cooldown == 0)
            {
                EnemyUseSkill1();
                usedSkill = true;
            }
