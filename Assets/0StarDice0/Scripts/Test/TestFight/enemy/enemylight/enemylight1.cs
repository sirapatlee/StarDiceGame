...
public class enemylight1: MonoBehaviour
{
    [Header("Player Setup")]
    ...
    private int enemyDef = 30; //def ของศัตรู
...
public PlayerState player;
    ...
    public Image showImage;
     public ItemID LightArmor = ItemID.LightArmor; // สมมติกล่องนี้ดรอปดาบไฟ
    public ItemID LightNeckless = ItemID.LightNeckless; 
    public ItemID LightRing = ItemID.LightRing; 
    ...
    public void OpenChest()
    {
        // เรียกใช้คำสั่งปลดล็อก
         int roll = Random.Range(1, 101);
            ...
            if (roll < 21)
            {
                 EquipmentManager.Instance.UnlockItem(LightArmor);
                  showImage.sprite = itemImages[0]; 
                 showImage.gameObject.SetActive(true);
            }
            else if (roll < 41 && roll > 20)
        {
             EquipmentManager.Instance.UnlockItem(LightNeckless);
              showImage.sprite = itemImages[1]; 
                 showImage.gameObject.SetActive(true);
        }
          else if (roll < 61 && roll > 40)
        {
             EquipmentManager.Instance.UnlockItem(LightRing);
 ...
 return; // ⭐️ สำคัญมาก: ออกจากฟังก์ชันนี้ทันที (ไม่ไปเปิดปุ่มด้านล่าง)
  ...
  GameEventManager.TryAddCount1(1);
    ...
    void ChooseAndExecuteBestEnemyMove(int basicAttackDamage)
    {
        ...
        float maxScore = Mathf.Max(scoreSkill1, scoreSkill2, scoreSkill3, scoreBasicAttack);
        ...
        {
            Debug.Log("AI เลือก: Skill 3 (dodge)");
            EnemySkill3();
        }
        else if (maxScore == scoreSkill2 && scoreSkill2 > -999)
        {
            Debug.Log("AI เลือก: Skill 2 (randomdark)");
            EnemySkill2();
        }
        else if (maxScore == scoreSkill1 && scoreSkill1 > -999)
        {
            Debug.Log("AI เลือก: Skill 1 (darkandheal)");
            EnemyUseSkill1();
        }
        else
        {
    ...
    float CalculateSkill1Score() // สกิล 1: โจมตี (Damage)
    {
        ...
        float score = 25; // ค่าพื้นฐานของดาเมจ

        // ถ้าผู้เล่นแพ้ทาง (ธาตุ dark) ให้คะแนนเยอะๆ
        if (selectedPlayer.elementType == ElementType.Dark)
            score *= 2; 
        ...
        if (isShieldActive || reflectNextAttackWind || doublereflectNextAttackWind > 0 || reflectNextAttack || shieldTurnsLeft > 0 || ReduceWater > 0|| superreduce >0 || isDodgeActive || isShieldActive || EarthBootDef >0 || lightShieldTurnsLeft >0 || superreducelight > 0|| EarthNerfDamage >0 || NerfEnemyDamgelight >0 ||ReduceLight>0)
    ...
    float CalculateSkill2Score() // สกิล 2 
    {
        if (enemySkill2Cooldown > 0) return -1000; // ใช้ไม่ได้

        float score = 30; // ค่าพื้นฐานของดาเมจ
    ...
    float CalculateSkill3Score() 
    {
       ...
       if (enemySkill3Cooldown > 0) return -1000; // ใช้ไม่ได้
        ...
        float score = 50; 
         ...
         if (enemyHP < (enemyMaxHP * 0.8f)) // ถ้าเลือดต่ำกว่า 80%
            score += 30; // ให้โบนัส
    ...
    {
        enemyHPBar.maxValue = 160;
        enemyHPBar.value = enemyHP;
        enemyHPText.text = $"HP: {enemyHP}/160";
    }
 ...
 void EnemyUseSkill1() //<--สกิล 1 ศัตรู
    {
        ...
        {
            damage *= 2;
            Debug.Log("ผู้เล่นเป็นธาตุ dark ดาเมจคูณ 2");
        }
        ...
        Debug.Log("ศัตรูใช้ dark ทำดาเมจ " + damage);
    }
    ...
    void EnemySkill3() //<--สกิล 3 ศัตรู
    {
        ...
        EnemyShieldWater100 = 1;
        enemySkill3Cooldown = 4;
         int healAmount =30;
        enemyHP += healAmount;
        enemyHP = Mathf.Clamp(enemyHP, 0, enemyMaxHP);
        UpdateEnemyHPUI();
