...
public class BattleSystemDark : MonoBehaviour
{
    [Header("Player Setup")]
...
StartCoroutine(DelayedEnemyTurn()); 
 ...
 GameEventManager.TryAddCount1(1);
    ...
    float CalculateSkill2Score() // สกิล 2:
    {
        ...
        float score = 35; // ค่าพื้นฐานของดาเมจ
    ...
    float CalculateSkill3Score() // สกิล 3: dodge
    {
        if (enemySkill3Cooldown > 0) return -1000; // ใช้ไม่ได้

        float score = 80; // ค่าพื้นฐานของ dodge
...
void EnemyUseSkill1() //<--สกิล 1 ศัตรู
    ...
    void EnemySkill2() //<--สกิล 2 ศัตรู
    {
        //ShowSkillEffectOnce(3); 
         int randomDamage = Random.Range(20, 61);
