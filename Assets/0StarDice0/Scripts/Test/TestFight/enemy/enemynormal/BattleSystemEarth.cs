...
public class BattleSystemEarth : MonoBehaviour
{
    [Header("Player Setup")]
...
public PlayerState player;
    ...
    public Image showImage;
     ...
     public ItemID DawnRing = ItemID.DawnRign; // สมมติกล่องนี้ดรอปดาบไฟ
...
StartCoroutine(DelayedEnemyTurn()); 
 ...
 GameEventManager.TryAddCount1(1);
    ...
    void ChooseAndExecuteBestEnemyMove(int basicAttackDamage)
    {
        ...
        {
            Debug.Log("AI เลือก: Skill 3 (Stun)");
            EnemySkill3();
        }
        else if (maxScore == scoreSkill2 && scoreSkill2 > -999)
        {
            Debug.Log("AI เลือก: Skill 2 (Shield)");
            EnemySkill2();
        }
    ...
    float CalculateSkill1Score() // สกิล 1: โจมตี (Damage)
    {
        ...
        // ถ้าผู้เล่นแพ้ทาง (ธาตุน้ำ) ให้คะแนนเยอะๆ
        if (selectedPlayer.elementType == ElementType.Water)
            score *= 2; 
        
        // ถ้าผู้เล่นชนะทาง (ธาตุลม) ให้คะแนนน้อยๆ
    ...
    float CalculateSkill2Score() // สกิล 2: โล่ (EnemyShieldEarth)
    {
        if (enemySkill2Cooldown > 0) return -1000; // ใช้ไม่ได้

        float score = 40; // ค่าพื้นฐานของการป้องกัน
        ...
        if (enemyHP < (enemyMaxHP * 0.25f)) // ถ้าเลือดต่ำกว่า 25%
            score += 60; // ให้โบนัสหนักๆ (อยากใช้ท่านี้สุดๆ)

        return score;
    }

    float CalculateSkill3Score() // สกิล 3: สตั๊น (EnemyStunPlayer)
    {
        if (enemySkill3Cooldown > 0) return -1000; // ใช้ไม่ได้

        // ถ้าผู้เล่นติดสตั๊นอยู่แล้ว (จาก EnemyTripleSkillTurn) อย่าใช้ซ้ำ
        if (playerStunTurns > 0 || EnemyStunPlayer > 0)
            return -500; // ใช้ซ้ำถือว่าโง่มาก

        float score = 50; // ค่าพื้นฐานของ Stun (มีประโยชน์มาก)

        // ถ้าศัตรู (AI) มี Cooldown สกิลอื่นที่อยากใช้เร็วๆ
        // การ Stun เพื่อซื้อเวลาถือว่าดี
    ...
    {
        enemyHPBar.maxValue = 150;
        enemyHPBar.value = enemyHP;
        enemyHPText.text = $"HP: {enemyHP}/150";
    }
    ...
    void EnemyUseSkill1() //<--สกิล 1 ศัตรู
    {
        ...
        {
            Debug.Log("ศัตรูใช้สกิล 1 ไม่ได้ รออีก " + enemySkill1Cooldown + " เทิร์น");
            return;
        }
        ...
        {
            damage /= 2;
            Debug.Log("ผู้เล่นเป็นธาตุลม ดาเมจลดลง 2 เท่า");
        }
        ...
        Debug.Log("ศัตรูทำดาเมจดิน " + damage);
    }
    ...
    void EnemySkill2() //<--สกิล 2 ศัตรู
    {
        ...
        enemySkill2Cooldown = 4;
        Debug.Log("ศัตรูใช้ shield Earth กันดาเมจ 100% 1 เทิร์น");
    }
    ...
    void EnemySkill3() //<--สกิล 3 ศัตรู
    {
        ...
        DamagePlayer(damage);
        PlaySoundEffect(5);
        Debug.Log("ศัตรูใช้สกิล 3 ดินทำให้ผู้เล่นไม่สามารถขยับได้หนึ่งเทิร์น พร้อมรับดาเมจ 10 ดาเมจ");
        ShowSkillEffectOnce(49);
        StartCoroutine(EnemyTripleSkillTurn());
