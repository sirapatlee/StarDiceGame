...
public class BattleSystemWind : MonoBehaviour
{
    [Header("Player Setup")]
...
// เติม [] ต่อท้ายเพื่อบอกว่าเป็น Array (เก็บได้หลายอัน)
    ...
    private int enemyDef = 35; //def ของศัตรู
...
public PlayerState player;
    ...
    {
        int expReward = 500; // จำนวน EXP ที่ต้องการให้
         ...
         if (lightBuffTurnsLeft > 0)//สกิล 2 แสง
    {
        ...
        enemyHP -= finalDamage;
        //enemyHP -= finalWaterDamage;
        enemyHP = Mathf.Clamp(enemyHP, 0, 130);
        StartCoroutine(ShakeEnemy()); // ศัตรูสั่น
...
StartCoroutine(DelayedEnemyTurn()); 
 ...
 GameEventManager.TryAddCount1(1);
    ...
    void ChooseAndExecuteBestEnemyMove(int basicAttackDamage)
    {
        ...
        {
            Debug.Log("AI เลือก: Skill 2 (randomdamage)");
            EnemySkill2();
        }
        else if (maxScore == scoreSkill1 && scoreSkill1 > -999)
        {
            Debug.Log("AI เลือก: Skill 1 (damage)");
            EnemyUseSkill1();
        }
        else
        {
    ...
    float CalculateSkill1Score() // สกิล 1: โจมตี (Damage)
    {
        ...
        float score = 20; // ค่าพื้นฐานของดาเมจ

        // ถ้าผู้เล่นแพ้ทาง (ธาตุ light) ให้คะแนนเยอะๆ
        if (selectedPlayer.elementType == ElementType.Earth)
            score *= 2; 
        
        else if (selectedPlayer.elementType == ElementType.Fire)
            score /= 2; 
        

        // ถ้าผู้เล่นมีโล่ (isShieldActive) หรือสะท้อน (reflectNextAttackWind)
        // การโจมตีจะได้ผลน้อยมาก หรือโดนสวน
        if (isShieldActive || reflectNextAttackWind || doublereflectNextAttackWind > 0 || reflectNextAttack || shieldTurnsLeft > 0 || ReduceWater > 0|| superreduce >0 || isDodgeActive || isShieldActive || EarthBootDef >0 || lightShieldTurnsLeft >0 || superreducelight > 0|| EarthNerfDamage >0 || NerfEnemyDamgelight >0 || ReduceWind >0)
    ...
    float CalculateSkill2Score() // สกิล 2 
    {
       if (enemySkill2Cooldown > 0) return -1000; // ใช้ไม่ได้
        ...
        // ถ้าผู้เล่นแพ้ทาง (ธาตุ earth) ให้คะแนนเยอะๆ
        if (selectedPlayer.elementType == ElementType.Earth)
            score *= 2; 
        ...
        // การโจมตีจะได้ผลน้อยมาก หรือโดนสวน
        if (isShieldActive || reflectNextAttackWind || doublereflectNextAttackWind > 0 || reflectNextAttack || shieldTurnsLeft > 0 || ReduceWater > 0|| superreduce >0 || isDodgeActive || isShieldActive || EarthBootDef >0 || lightShieldTurnsLeft >0 || superreducelight > 0|| EarthNerfDamage >0 || NerfEnemyDamgelight >0 || ReduceWind >0)
            score = 4; // ให้คะแนนน้อยสุดๆ (แต่ยังดีกว่าติดลบ)
    ...
    float CalculateSkill3Score() // สกิล 3 reflect
    {
        ...
        if (enemyHP < (enemyMaxHP * 0.25f)) // ถ้าเลือดต่ำกว่า 25%
            score += 70; // ให้โบนัสหนักๆ (อยากใช้ท่านี้สุดๆ)
    ...
    {
        enemyHPBar.maxValue = 130;
        enemyHPBar.value = enemyHP;
        enemyHPText.text = $"HP: {enemyHP}/130";
    }
 ...
 void EnemyUseSkill1() //<--สกิล 1 ศัตรู
    {
        ...
        if (selectedPlayer.elementType == ElementType.Fire)
        {
            damage /= 2;
            Debug.Log("ผู้เล่นเป็นธาตุไฟ ดาเมจถูกลดครึ่งหนึ่ง");
        }
        else if (selectedPlayer.elementType == ElementType.Earth)
        {
            damage *= 2;
    ...
    void EnemySkill2() //<--สกิล 2 ศัตรู
    {
        ...
        Debug.Log("ศัตรูใช้ Super Wind ทำดาเมจ " + damage);
    }
    ...
    void EnemySkill3() //<--สกิล 3 ศัตรู
    {
        ...
        Debug.Log($"ศัตรูใช้สกิล 3 สะท้อนดาเมจผู้เล่น {EnemyReflectWind} เทิร์น");
