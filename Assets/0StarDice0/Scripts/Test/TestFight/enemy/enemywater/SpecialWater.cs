...
public class SpecialWater : MonoBehaviour
{
    [Header("Player Setup")]
    ...
    private int enemyDef = 60; //def ของศัตรู
...
StartCoroutine(DelayedEnemyTurn()); 
 ...
 return; // ⭐️ สำคัญมาก: ออกจากฟังก์ชันนี้ทันที (ไม่ไปเปิดปุ่มด้านล่าง)
    ...
    float CalculateSkill2Score() // สกิล 2: โจมตี (Damage)
    {
       ...
       if (enemyHP > (enemyMaxHP * 0.85f)) // ถ้าเลือดมากกว่า 85%
            score -= 40; 
    ...
    {
        enemyHPBar.maxValue = 320;
        enemyHPBar.value = enemyHP;
        enemyHPText.text = $"HP: {enemyHP}/320";
    }
 ...
 void EnemyUseSkill1() //<--สกิล 1 ศัตรู
    ...
    void EnemySkill3() //<--สกิล 3 ศัตรู
    {
        ...
        int healAmount = Random.Range(20, 61);;
        enemyHP += healAmount;
        enemyHP = Mathf.Clamp(enemyHP, 0, enemyMaxHP);
        ShowSkillEffectOnce(3);
        PlaySoundEffect(2);
