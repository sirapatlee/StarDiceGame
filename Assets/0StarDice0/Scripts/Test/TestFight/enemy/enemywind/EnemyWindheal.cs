...
public class EnemyWindheal: MonoBehaviour
{
    [Header("Player Setup")]
...
public PlayerState player;
    ...
    public Image showImage;     
     public ItemID WindShoes = ItemID.WindShoes; // สมมติกล่องนี้ดรอปดาบไฟ
        public ItemID WindEye = ItemID.WindEye;
    public ItemID WindSword = ItemID.WindSword;
    ...
    public void OpenChest()
    {
        // เรียกใช้คำสั่งปลดล็อก
         int roll = Random.Range(1, 101);
            ...
            if (roll < 21)
            {
                 EquipmentManager.Instance.UnlockItem(WindShoes);
                   showImage.sprite = itemImages[0]; 
                 showImage.gameObject.SetActive(true);
            }
            else if (roll < 41 && roll > 20)
        {
             EquipmentManager.Instance.UnlockItem(WindEye);
               showImage.sprite = itemImages[1]; 
                 showImage.gameObject.SetActive(true);
        }
          else if (roll < 61 && roll > 40)
        {
             EquipmentManager.Instance.UnlockItem(WindSword);
...
StartCoroutine(DelayedEnemyTurn()); 
 ...
 return; // ⭐️ สำคัญมาก: ออกจากฟังก์ชันนี้ทันที (ไม่ไปเปิดปุ่มด้านล่าง)
  ...
  GameEventManager.TryAddCount1(1);
    ...
    void ChooseAndExecuteBestEnemyMove(int basicAttackDamage)
    {
        ...
        {
            Debug.Log("AI เลือก: Skill 3 (budd)");
            EnemySkill3();
        }
        ...
        {
            Debug.Log("AI เลือก: Skill 1 (earth)");
            EnemyUseSkill1();
        }
        else
        {
    ...
    float CalculateSkill2Score() // สกิล 2 
    {
        ...
        float score = 50; 
         ...
         if (enemyHP > (enemyMaxHP * 0.7f)) // ถ้าเลือดมากกว่า 70%
            score -= 50; // ให้โบนัส
 ...
 void EnemyUseSkill1() //<--สกิล 1 ศัตรู
    ...
    void EnemySkill3() //<--สกิล 3 ศัตรู
    {
        ...
        Debug.Log($"ศัตรูใช้สกิล reflect");
